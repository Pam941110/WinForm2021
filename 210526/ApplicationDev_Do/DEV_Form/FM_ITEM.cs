...
namespace DEV_Form
{
    public partial class FM_ITEM : Form , ChildInterFace
    {
        ...
        private void btnLoadPic_Click(object sender, EventArgs e)
        {
            ...
            OpenFileDialog Dialog = new OpenFileDialog(); // static이 없으므로 객체 이렇게 생성해줘야 함. 
        ...
        private void btnPicSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sItemCode = dgvGrid.CurrentRow.Cells["ITEMCODE"].Value.ToString();
                cmd.CommandText = "UPDATE TB_TESTITEM_DSH SET ITEMIMG = @IMAGE " + 
        ...
        private void dgvGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                string sSql = "SELECT ITEMIMG FROM TB_TESTITEM_DSH WHERE ITEMCODE = '" + sItemCode + "'AND ITEMIMG IS NOT NULL";
                SqlDataAdapter Adapter = new SqlDataAdapter(sSql, Connect);
                DataTable dtTemp = new DataTable();
                Adapter.Fill(dtTemp);
                ...
                {
                    byte[] bImage = null;
                    bImage = (byte[])dtTemp.Rows[0]["ITEMIMAGE"]; // 이미지를 byte화 한다. (형변환)
                    if (bImage != null)
                    {
                        picitemimage.Image = new Bitmap(new MemoryStream(bImage)); // 메모리 스트림을 이용하여
                        picitemimage.BringToFront();
                    }
                }
            }
        ...
        private void btnPicDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string sItemCode = dgvGrid.CurrentRow.Cells["ITEMCODE"].Value.ToString();
                cmd.CommandText = "UPDATE TB_TESTITEM_DSH SET ITEMIMG = null WHERE ITEMCODE ='"+sItemCode;
                cmd.Connection = Connect;
                cmd.ExecuteNonQuery();
                picitemimage.Image = null;
                MessageBox.Show("정상적으로 삭제 하였습니다.");
